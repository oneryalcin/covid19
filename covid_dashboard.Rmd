...
df <- df %>%  
  rename(cases=`Daily lab-confirmed cases`, day =`Specimen date`, area_name='Area name', area_type = `Area type`, id = `Area code`)
...
cases_per_regions = df %>% filter(area_type == "Region") %>% group_by(date_range, area_name) %>% summarise(total = sum(cases))
...
compare_weeks = df %>% 
  ...
  group_by(area_name, id, date_range) %>% 
  summarise(total = sum(cases)) %>% 
...
cases_2wago_per_area = df %>% filter(date_range == 'past7-14days', area_type == "Lower tier local authority") %>% group_by(area_name) %>% summarise(total = sum(cases))
...
gg <- ggplot() + geom_polygon(data = mapdata, aes(x = long, y = lat, group = group, fill = diff), color = "#FFFFFF", size = 0.25)
gg <- gg + coord_fixed(1)
ggplotly(gg)
